                    ...
                    try {
                      ...
                      try {
                        ...
                        if (!error && data?.html) {
                          ...
                          console.warn("Edge function failed, using client-side fallback:", error);
                        }
                      } catch (edgeErr) {
                        console.warn("Edge function error, using client-side fallback:", edgeErr);
                      }

                      // Client-side fallback
                      if (!html) {
                        const clientName = [viewSig.client?.first_name, viewSig.client?.last_name].filter(Boolean).join(" ") || "Patient";
                        const signedDate = viewSig.signed_at ? new Date(viewSig.signed_at).toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" }) : "N/A";
                        html = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>${viewSig.template?.name ?? "Consent Form"}</title><style>@page{margin:1in}body{font-family:'Segoe UI',sans-serif;color:#1a1a1a;line-height:1.6;max-width:800px;margin:0 auto;padding:40px}.header{border-bottom:3px solid #9333EA;padding-bottom:16px;margin-bottom:24px}.header h1{color:#9333EA;margin:0;font-size:24px}.patient-info{background:#f8f8fa;border-radius:8px;padding:16px;margin-bottom:24px}.patient-info p{margin:4px 0;font-size:14px}.patient-info strong{display:inline-block;width:140px;color:#555}.consent-body{border:1px solid #e5e5e5;border-radius:8px;padding:24px;margin-bottom:24px}.signature-section{background:#faf8ff;border:2px solid #9333EA33;border-radius:8px;padding:24px;margin-bottom:24px;page-break-inside:avoid}.signature-section h2{color:#9333EA;margin-top:0}.sig-img{max-height:100px;border:1px solid #ddd;border-radius:4px}.audit-section{background:#f0f0f4;border-radius:8px;padding:20px;font-size:12px;page-break-inside:avoid}.footer{text-align:center;color:#999;font-size:11px;border-top:1px solid #eee;padding-top:16px;margin-top:32px}@media print{body{padding:0}}</style></head><body><div class="header"><h1>${viewSig.template?.name ?? "Consent Form"}</h1></div><div class="patient-info"><p><strong>Patient:</strong> ${clientName}</p><p><strong>Status:</strong> ${(viewSig.status ?? "—").toUpperCase()}</p><p><strong>Signed:</strong> ${signedDate}</p><p><strong>Version:</strong> v${viewSig.template_version ?? 1}</p></div><div class="consent-body"><h2>Consent Document</h2>${viewSig.signed_html_snapshot ?? viewSig.template?.body_html ?? ""}</div><div class="signature-section"><h2>Patient Signature</h2>${viewSig.signature_canvas_data ? `<img src="${viewSig.signature_canvas_data}" alt="Signature" class="sig-img">` : ""}${viewSig.signature_typed_name ? `<p><strong>Typed Name:</strong> ${viewSig.signature_typed_name}</p>` : ""}<p><strong>Confirmed:</strong> ${viewSig.signature_checkbox_confirmed ? "✓ Yes" : "✗ No"}</p></div><div class="audit-section"><h2>Legal Audit Trail</h2><p><strong>IP Address:</strong> ${viewSig.signer_ip_address ?? "Not captured"}</p><p><strong>User Agent:</strong> ${viewSig.signer_user_agent ?? "Not captured"}</p></div><div class="footer">Generated by ClinicPro · ${new Date().toLocaleString()} · ID: ${viewSig.id}</div></body></html>`;
                      }
                      ...
                      console.error("PDF generation full error:", err);
                      toast.error(`PDF Error: ${err?.message ?? String(err).slice(0, 100)}`);
                    }
                  }}
                >
