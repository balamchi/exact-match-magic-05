...
interface Inv { id: string; client_name: string; total_cents: number; status: string; created_at: string; due_on: string | null }
...
function ARAging() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase.from("invoices")
        .select("id, client_name, total_cents, status, created_at, due_on" as never)
  ...
  const bucket = (i: Inv) => {
    const days = (Date.now() - new Date(i.due_on ?? i.created_at).getTime()) / 86400000;
    ...
    <>
      ...
      <ReportShell
        ...
        onExport={() => exportToCsv(reportFilename("ar-aging", "csv"),
          ...
          rows.map((r) => [r.id, r.client_name, r.status, r.created_at, r.due_on ?? "", bucket(r), (r.total_cents / 100).toFixed(2)]))}
      >
        <ReportTable
          loading={loading}
          columns={[
            ...
            { key: "age", header: "Age", cell: (r) => daysAgo(r.due_on ?? r.created_at) },
